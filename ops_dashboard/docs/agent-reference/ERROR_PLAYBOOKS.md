...
<a id="cq08"></a>
| **CQ08** | 상품 이미지 중복(상품당 N장, 규격 1장) | **이중삽입**: AI가 h3 블록에 마크다운 이미지 작성 + 후처리(5)가 같은 상품에 figure 또 삽입 → figure 변환·gallery 래핑으로 2장 렌더(동일 이미지의 R2 복제본, md5 증명) | _normalize_product_blocks (5)에서 방식A/B 매칭 성공 시 해당 h3 구간의 순수 마크다운 이미지 줄 제거 → 상품당 figure 1장. url 미매칭 폴백 시 AI 이미지 유지(0장 방지) | 룩북 CQ08 + CQ08 재검사 트리거 | 설명 문장 줄은 이미지 줄과 분리 — 이미지 줄만 제거하면 단락 보존(CQ07 유지) |
