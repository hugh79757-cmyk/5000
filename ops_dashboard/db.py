...
SEED_ISSUES: list[dict] = [
    ...
    # Publish reliability hotfix 후속 (2026-08-17) — 배포 코드에 섞지 않고 별도 P1로 등록
    {"issue_id": "DOUBLE-FAILURE-COUNT-INCREMENT", "blog_ids": "", "category": "structural", "symptom": "dispatcher.py 동일 execution에서 _increment_failure_count가 line 1405/1417 경로를 통해 두 번 호출될 가능성 — failure_count/alert threshold/cooldown 이중 증분 위험", "recorded_date": "2026-08-17", "gsd_status": "open", "auto_detectable": "no", "detection_method": "dispatcher.py 실패 처리 경로 코드 리뷰 — _increment_failure_count 2회 호출 지점 확인", "notes": "필수 후속 조사: (1) 동일 execution_id에서 실제 2회 증가하는지 (2) 어떤 return/exception 경로에서 중복되는지 (3) failure_count, alert threshold, cooldown에 미치는 영향 (4) 단일 owner 함수로 통합 가능한지 (5) P01/P14/P16 회귀 테스트. 발견 경위: publish-reliability hotfix GATE 7 관찰 중 dispatcher 실패 경로 조사 (2026-08-17)."},
    {"issue_id": "STRUCT-19", "blog_ids": "", "category": "structural", "symptom": "execution_id 기반 occurrence 정밀 dedup 미구현 — 동일 execution이 notifier/monitor 두 recorder 경로로 기록되면 incident_key upsert 병합은 되지만 occurrence_count가 +1씩 누적됨", "recorded_date": "2026-08-17", "gsd_status": "open", "auto_detectable": "no", "detection_method": "publish-error-events 동일 execution 양 경로 발생 시 occurrence_count 누적 관찰", "notes": "publish-reliability hotfix 커밋 B에서 incident_key 병합은 해결했으나 occurrence_count 중복은 허용 오차로 둠. 정밀 dedup은 execution_id 도입(migration) 필요 — 별도 승인 후 진행. 발견 경위: 2026-08-17 hotfix GATE 0-B taxonomy 검증."},
    {"issue_id": "STRUCT-20", "blog_ids": "pick-hugo", "category": "structural", "symptom": "top5_rank/price_trend effective availability 미적용 — persona_pick에만 eligibility 게이트(SSOT 헬퍼) 적용, 다른 car post_type은 raw pending 카운트 유지", "recorded_date": "2026-08-17", "gsd_status": "open", "auto_detectable": "no", "detection_method": "candidate_availability._count_pending post_type 분기 코드 리뷰", "notes": "publish-reliability hotfix stabilization 커밋에서 persona_pick에만 적용(사용자 지시 범위). top5_rank/price_trend 빌더 요건 파악 후 동일 SSOT 패턴 확장 검토. 발견 경위: 2026-08-17 hotfix GATE 0-A parity 조사."},
    {"issue_id": "STRUCT-21", "blog_ids": "", "category": "structural", "symptom": "운영 경로 하드코딩 테스트 5개 — tests/curation/ (test_collector, test_enricher, test_keywords, test_pipeline, test_title_regression)이 sys.path.insert(0, '/Users/twinssn/Projects/5000')로 운영 코드+운영 DB를 import — worktree 테스트 실행 시 운영 오염 위험", "recorded_date": "2026-08-17", "gsd_status": "open", "auto_detectable": "no", "detection_method": "grep sys.path.insert 하드코딩 경로 — tests/curation 5파일 확인", "notes": "publish-reliability hotfix GATE 1에서 발견. 운영 DB 쓰기 오염은 미확인(mtime 갱신은 실시간 운영 활동+조회 부수 효과)이나 경로 노출 위험 상존. PROJECT_ROOT 상대 경로로 교체 필요. 발견 경위: 2026-08-17 hotfix GATE 1 테스트 격리 검증."},
    {"issue_id": "STRUCT-22", "blog_ids": "", "category": "structural", "symptom": "baseline 환경 의존 실패 24개 미정리 — 운영 DB/API 키 부재로 실패하는 테스트 24개가 nodeid 고정으로 계속 빨간불", "recorded_date": "2026-08-17", "gsd_status": "open", "auto_detectable": "no", "detection_method": "GATE 1 delta — baseline/hotfix 공통 실패 nodeid 24개 동일 확인", "notes": "publish-reliability hotfix GATE 1 회귀 delta에서 baseline 325 passed/24 failed 확인 — 회귀 0과는 무관(사전 존재). pytest 마커(skip) 또는 fixture 분리로 정리 필요. 발견 경위: 2026-08-17 hotfix GATE 1."},
]
