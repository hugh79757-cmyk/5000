"""여행(TAP) 파이프라인 표준 골격 — validator (Phase 61, D-01).

표준 6-모듈 골격(pipeline/fetcher/topic_manager/writer/enrich/validator)의 일부.
travel 분기는 독립적인 validator 모듈이 없었고, 검증이 `shared.validators` 의
`validate_post_extended(..., pipeline="travel")` 호출로 pipeline.py 안에 인라인되어 있다
(pipeline.py:333).
...
def validate_post(cfg, article):
    ...
    issues = validate_post_extended(blog_id, title, body_md, ctx, pipeline="travel")
