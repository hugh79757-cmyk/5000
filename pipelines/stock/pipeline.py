...
def run(blog_cfg):
    ...
    if strategy == "disclosure":
        ...
        if article.get("title") and article.get("body_md"):
            ...
            # ── source_id 고유 생성 (기업코드 조합 + 날짜) ──
            _corp_codes = sorted([c.get("stock_code","") or c.get("corp_code","") for c in (enriched or sample)])
            _source_id = f"{topic_type}_{'_'.join(_corp_codes[:5])}_{datetime.now().strftime('%Y%m%d')}"
            result = publish(
                blog_id=blog_id,
                title=article["title"],
                body_md=_body_eg,
