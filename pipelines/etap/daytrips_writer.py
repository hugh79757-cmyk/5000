...
Return ONLY the article in markdown starting with # title"""
    ...
    resp = _get_client().chat.completions.create(
        ...
        messages=[
            {"role":"system","content":"You are a travel blogger who has actually visited these destinations. Write in first-person-informed tone (not literally 'I did X' but 'the 20-minute taxi ride from downtown drops you right at the entrance'). STRICT RULES: 1) Never use these words/phrases: plethora, vibrant, bustling, let\'s dive in, without further ado, hidden gem, tapestry, myriad, embark, crystal-clear, soak in, immerse yourself. 2) Format prices as whole numbers when .0 (write $8 not $8.0, write $12 not $12.0). 3) Never invent data. 4) Every section must include at least one practical tip (best time of day, what to wear, how to get there, what to skip). 5) Open with a specific, concrete hook - a scene, a number, a surprising fact - not a generic overview sentence."},
