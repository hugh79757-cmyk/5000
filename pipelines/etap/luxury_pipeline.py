"""luxury_pipeline.py - Luxury And Private Tours pipeline"""
import logging
import os
import re
import subprocess
...
from pipelines.etap.luxury_writer import generate_luxury_guide
...
BLOG_ID = "luxury-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/luxury-hugo"
TOPIC_TABLE = "luxury_topics"
...
def _run_impl(cfg=None) -> bool:
    ...
    article = generate_luxury_guide(topic)
    if not article:
    ...
    cover = fetch_city_image(city + " luxury private tour", country, article["slug"]) if city else None
    body = fetch_body_images(city + " luxury travel experience", country, article["slug"], count=8) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "luxury and private tours in " + city, 50, 1)
    if country:
