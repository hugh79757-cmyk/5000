"""extreme_writer.py - Extreme Sports And Adventure Tours guide generator"""
...
def fetch_tours(city, country=None):
    ...
    rows = conn.execute("""
        ...
        WHERE (city = ? OR city IN (SELECT alias FROM city_aliases WHERE canonical_name = ?))
          AND category IN ('Extreme Sports', 'White Water Rafting', 'Rafting', 'Ziplining', 'Paragliding', 'Climbing')
...
def generate_extreme_guide(topic):
    city = topic["city"]
    ...
    h2s = """  ## Why {city} Is an Extreme Sports Destination
  ## Top Extreme Sports in {city}
  ## Rafting and Water Adventures
  ## Ziplining Paragliding and Air Sports
  ## Prices Safety and Booking Tips
  ## Best Time for Extreme Sports in {city}"""
    prompt = f"""Write a extreme sports and adventure tours guide for {city}, {country}.
...
- Title must include "{city}" and relate to extreme sports and adventure tours
...
- Write as a adrenaline sports coach and adventure travel blogger
...
Return ONLY the article in markdown starting with # title"""
    resp = _get_client().chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a adrenaline sports coach and adventure travel blogger. STRICT RULES: 1) Never use: vibrant, bustling, hidden gem, treasure trove, must-visit, immerse yourself, embark, crystal-clear, world-class, bucket list, plethora, tapestry, myriad. 2) Format prices as whole numbers. 3) Never invent data. 4) Every section must include one practical tip. 5) Open with a specific concrete scene or fact."},
            {"role": "user", "content": prompt}
        ]
    )
    content = resp.choices[0].message.content.strip()
    title_match = re.match(r"^#\s+(.+)", content)
    title = title_match.group(1).strip() if title_match else topic.get("title", f"Extreme Sports And Adventure Tours in {city}")
    content = re.sub(r"^#\s+.+\n*", "", content, count=1).strip()
    slug = topic.get("slug", re.sub(r"[^a-z0-9]+", "-", city.lower()).strip("-") + "-extreme-tours")
    tags = [city, country, "Extreme Sports"] if country else [city, "Extreme Sports"]
    return {"title": title, "slug": slug, "content": content,
             "description": f"Best extreme sports and adventure tours in {city}: prices, top picks, and practical tips.",
