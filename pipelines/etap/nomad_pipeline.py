"""nomad_pipeline.py - Digital Nomad Guide And Coworking pipeline"""
...
from pipelines.etap.nomad_writer import generate_nomad_guide
...
BLOG_ID = "nomad-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/nomad-hugo"
TOPIC_TABLE = "nomad_topics"
...
def run(cfg=None):
    ...
    article = generate_nomad_guide(topic)
    if not article:
    ...
    body = fetch_body_images(city + " coworking cafe laptop work", country, article["slug"], count=3) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "digital nomad guide and coworking in " + city, 50, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "digital nomad guide and coworking in " + country, 35, 1)
    return True
