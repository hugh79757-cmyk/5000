"""luxury_writer.py - Luxury And Private Tours guide generator"""
...
def fetch_tours(city, country=None):
    ...
    rows = conn.execute("""
        ...
        WHERE (city = ? OR city IN (SELECT alias FROM city_aliases WHERE canonical_name = ?))
          AND category IN ('Private and Luxury')
...
def generate_luxury_guide(topic):
    city = topic["city"]
    ...
    h2s = """  ## Why Choose Private and Luxury Tours in {city}
  ## Top Private Tours in {city}
  ## Luxury Day Trips and Excursions
  ## Prices and What to Expect
  ## Tips for Booking Luxury Tours in {city}"""
    prompt = f"""Write a luxury and private tours guide for {city}, {country}.
...
- Title must include "{city}" and relate to luxury and private tours
...
- Write as a luxury travel specialist who has experienced private tours worldwide
...
Return ONLY the article in markdown starting with # title"""
    ...
    slug = topic.get("slug", re.sub(r"[^a-z0-9]+", "-", city.lower()).strip("-") + "-luxury-tours")
    tags = [city, country, "Luxury Tours"] if country else [city, "Luxury Tours"]
    return {"title": title, "slug": slug, "content": content,
             "description": f"Best luxury and private tours in {city}: prices, top picks, and practical tips.",
