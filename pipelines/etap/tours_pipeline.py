...
from pipelines.etap.tours_writer import generate_tours_guide

BLOG_ID = "tours-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/tours-hugo"
TOPIC_TABLE = "tours_topics"
...
def pick_topic():
    conn = _get_db()
    row = conn.execute(
        "SELECT * FROM tours_topics WHERE exhausted = 0 "
        "AND slug NOT IN (SELECT slug FROM publish_log WHERE blog_id = 'tours-hugo') "
...
def _add_product_cards(article):
    ...
    if selected:
        article["content"] = insert_product_cards(article["content"], selected, max_cards=10)
    return article
...
def run():
    ...
    if not topic:
        ...
        return False
    logger.info(f"[tours-hugo] {topic.get('city','')} generating")
...
def run_batch(count=3):
    ...
    logger.info(f"[tours-hugo] Batch {ok}/{count}")
    return ok
