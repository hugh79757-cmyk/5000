"""hiking_pipeline.py - Hiking And Mountain Bike Tours pipeline"""
...
from pipelines.etap.hiking_writer import generate_hiking_guide
...
BLOG_ID = "hiking-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/hiking-hugo"
TOPIC_TABLE = "hiking_topics"
...
def run(cfg=None):
    ...
    article = generate_hiking_guide(topic)
    if not article:
    ...
    cover = fetch_city_image(city + " hiking trail mountain", country, article["slug"]) if city else None
    body = fetch_body_images(city + " hiking mountain bike trail", country, article["slug"], count=8) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "hiking and mountain bike tours in " + city, 50, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "hiking and mountain bike tours in " + country, 35, 1)
    return True
