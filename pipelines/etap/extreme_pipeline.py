"""extreme_pipeline.py - Extreme Sports And Adventure Tours pipeline"""
...
from pipelines.etap.extreme_writer import generate_extreme_guide

logger = logging.getLogger(__name__)
KST = timezone(timedelta(hours=9))
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

BLOG_ID = "extreme-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/extreme-hugo"
TOPIC_TABLE = "extreme_topics"
...
def run(cfg=None):
    ...
    article = generate_extreme_guide(topic)
    if not article:
    ...
    cover = fetch_city_image(city + " extreme sports adventure", country, article["slug"]) if city else None
    body = fetch_body_images(city + " extreme sports rafting zipline", country, article["slug"], count=8) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "extreme sports and adventure tours in " + city, 50, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "extreme sports and adventure tours in " + country, 35, 1)
    return True
