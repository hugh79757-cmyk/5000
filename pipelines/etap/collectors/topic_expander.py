...
def _make_slug(prefix, city) -> str:
    s = city.lower().strip()
...
def expand_topics():
    ...
    for blog, cfg in BLOG_CATEGORIES.items():
        ...
        # 기존 도시 + 슬러그
        existing_cities = {r[0] for r in conn.execute(f"SELECT DISTINCT city FROM {tbl}").fetchall()}
        existing_slugs = set()
        if "slug" in cols:
            existing_slugs = {r[0] for r in conn.execute(f"SELECT DISTINCT slug FROM {tbl}").fetchall()}
