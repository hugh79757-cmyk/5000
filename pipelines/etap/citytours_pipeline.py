"""citytours_pipeline.py - City Tours And Sightseeing pipeline"""
import logging
import os
import re
import subprocess
...
from pipelines.etap.citytours_writer import generate_citytours_guide
...
BLOG_ID = "citytours-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/citytours-hugo"
TOPIC_TABLE = "citytours_topics"
CATEGORY = "City Tours"
...
def run(cfg=None) -> bool:
    ...
    article = generate_citytours_guide(topic)
    if not article:
    ...
    cover = fetch_city_image(city + " city tour sightseeing", country, article["slug"]) if city else None
    body = fetch_body_images(city + " city sightseeing tour", country, article["slug"], count=8) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "city tours and sightseeing in " + city, 50, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "city tours and sightseeing in " + country, 35, 1)
    return True
