...
def _run_impl() -> bool:
    ...
    body = fetch_body_images(city + " adventure activity", country, article["slug"], count=8) if city else []
...
def run_batch(count=1):
    ...
    can_pub, today_count = check_daily_quota("adventure-hugo", max_per_day=5)
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[adventure-hugo] daily quota reached ({today_count}/5)")
        return 0
