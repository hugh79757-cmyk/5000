"""layover_pipeline.py - Layover Tours And Stopover Guides pipeline"""
...
from pipelines.etap.layover_writer import generate_layover_guide

logger = logging.getLogger(__name__)
KST = timezone(timedelta(hours=9))
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

BLOG_ID = "layover-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/layover-hugo"
TOPIC_TABLE = "layover_topics"
CATEGORY = "Layover Tours"
...
def run(cfg=None):
    ...
    article = generate_layover_guide(topic)
    if not article:
    ...
    cover = fetch_city_image(city + " layover city tour airport", country, article["slug"]) if city else None
    body = fetch_body_images(city + " layover stopover city tour", country, article["slug"], count=8) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "layover tours and stopover guides in " + city, 50, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "layover tours and stopover guides in " + country, 35, 1)
    return True
