"""nature_pipeline.py - Nature Tours blog pipeline"""
...
from pipelines.etap.nature_writer import generate_nature_guide

BLOG_ID = "nature-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/nature-hugo"
TOPIC_TABLE = "nature_topics"
...
def run():
    ...
    article = generate_nature_guide(topic)
    if not article:
    ...
    body = fetch_body_images(city + " nature tour", country, article["slug"], count=3) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"],
                        "nature tours from " + city, 60, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"],
                        "nature tours in " + country, 40, 1)
    return True

def run_batch(count=1):
    from pipelines.etap.topic_manager import check_daily_quota
    can_pub, today_count = check_daily_quota("nature-hugo", max_per_day=5)
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[nature-hugo] daily quota reached ({today_count}/5)")
        return 0
