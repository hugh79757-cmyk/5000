...
def pick_topic():
    conn = _get_db()
    row = conn.execute(
        "SELECT * FROM visa_topics WHERE exhausted = 0 "
        "AND slug NOT IN (SELECT slug FROM publish_log WHERE blog_id = 'visa-hugo') "
...
def _add_product_cards(article):
    ...
    conn.close()
    if cross:
        article["content"] = insert_product_cards(article["content"], cross, max_cards=4, position="after_h2")
    return article
