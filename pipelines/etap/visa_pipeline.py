...
from pipelines.etap.visa_writer import generate_visa_guide
...
def _run_impl() -> bool:
    topic = pick_topic()
    if not topic:
        logger.info("[visa-hugo] No topics")
        return False
    logger.info(f"[visa-hugo] {topic.get('passport','')} generating")
    ...
    # cross-sell (base pipeline.py:333-339 클론 — F5 Golden Standard 픽스)
    cross_html = build_cross_sell_html(
        country=article.get("country", ""),
        city=topic.get("passport", ""),
...
def run_batch(count=3):
    ...
    can_pub, today_count = check_daily_quota("visa-hugo", max_per_day=5)
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[visa-hugo] daily quota reached ({today_count}/5)")
        return 0
    ...
    for _ in range(count):
        if _run_impl():
            ok += 1
        time.sleep(5)
    logger.info(f"[visa-hugo] Batch {ok}/{count}")
    return ok
