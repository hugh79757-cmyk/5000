"""cruise_writer.py - Shore Excursion guide generator (v2: curated, deduplicated)"""
import logging
import os
import re
import sqlite3
...
def generate_cruise_guide(topic):
    city = topic["city"]
    ...
    logger.info(f"[cruise] {city}: {original_count} tours -> {dedup_count} after dedup")
    ...
    prompt = f"""Write a shore excursions guide for cruise passengers visiting {city}, {country}.
...
## [Hook: one specific sentence about arriving at {city} port]
## Top Shore Excursions in {city}
## Best Half-Day Port Tours
## Full-Day Excursions Worth the Splurge
## Tips for Cruise Passengers in {city}

WRITING RULES:
1. OPENING: Start with a concrete hook. Example style: "A 20-minute taxi from downtown drops you at the foot of 4,500-year-old pyramids, the driver waving you toward the gangplank." Do NOT start with generic overview sentences. Do NOT attach any invented transport or entry price to the scene.
...
8. NEVER use: plethora, vibrant, bustling, embark, tapestry, myriad, hidden gem, unforgettable, crystal-clear, soak in, immerse yourself, lets dive in, without further ado, a testament to, culinary delights, gastronomic, rich cultural heritage, seamlessly, breathtaking, brimming with, a must-visit, treasure trove, staggering"""
    ...
    title = title_match.group(1).strip() if title_match else topic.get("title", f"Best Shore Excursions from {city}")
    content = re.sub(r"^#\s+.+\n*", "", content, count=1).strip()

    slug = topic.get("slug", re.sub(r"[^a-z0-9]+", "-", city.lower()).strip("-"))
    tags = [city, country, "Shore Excursions", "Travel"] if country else [city, "Shore Excursions", "Travel"]

    return {
        "title": title,
        "slug": slug,
        "content": content,
        "description": f"Best shore excursions in {city} for cruise passengers: port tours with real prices, timing tips, and honest comparisons.",
