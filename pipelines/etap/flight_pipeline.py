...
def run(cfg):
    ...
    article["content"], _post_issues, _is_draft = postprocess_content(article["content"], None, blog_id)
    if _is_draft:
        logger.warning(f"[{blog_id}] DRAFT 감지 → 발행 중단: {article['slug']} - {_post_issues}")
