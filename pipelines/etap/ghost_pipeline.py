...
BLOG_ID = "ghost-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/ghost-hugo"
TOPIC_TABLE = "ghost_topics"
...
def run(cfg=None):
    ...
    article = generate_ghost_guide(topic)
    if not article:
    ...
    body = fetch_body_images(city + " ghost tour haunted city", country, article["slug"], count=3) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "ghost tours and dark history in " + city, 50, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "ghost tours and dark history in " + country, 35, 1)
    return True
