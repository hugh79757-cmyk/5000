"""citytours_writer.py - City Tours And Sightseeing guide generator"""
...
def fetch_tours(city, country=None):
    ...
    rows = conn.execute("""
        ...
        WHERE (city = ? OR city IN (SELECT alias FROM city_aliases WHERE canonical_name = ?))
          AND category IN ('City Tours', 'Audio Guides', 'Self-guided Tours')
...
def generate_citytours_guide(topic):
    city = topic["city"]
    ...
    h2s = """  ## Best Ways to See {city} on a City Tour
  ## Top City Tours in {city}
  ## Audio Guides and Self-Guided Options
  ## Prices and Booking Tips
  ## Making the Most of Your City Tour in {city}"""
    prompt = f"""Write a city tours and sightseeing guide for {city}, {country}.
...
- Write as a city explorer and travel writer who knows every neighborhood
...
Return ONLY the article in markdown starting with # title"""
    resp = _get_client().chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a city explorer and travel writer who knows every neighborhood. STRICT RULES: 1) Never use: vibrant, bustling, hidden gem, treasure trove, must-visit, immerse yourself, embark, crystal-clear, world-class, bucket list, plethora, tapestry, myriad. 2) Format prices as whole numbers. 3) Never invent data. 4) Every section must include one practical tip. 5) Open with a specific concrete scene or fact."},
            {"role": "user", "content": prompt}
        ]
    )
    content = resp.choices[0].message.content.strip()
    title_match = re.match(r"^#\s+(.+)", content)
    title = title_match.group(1).strip() if title_match else topic.get("title", f"City Tours And Sightseeing in {city}")
    content = re.sub(r"^#\s+.+\n*", "", content, count=1).strip()
    slug = topic.get("slug", re.sub(r"[^a-z0-9]+", "-", city.lower()).strip("-") + "-city-tours")
    tags = [city, country, "City Tours"] if country else [city, "City Tours"]
    return {"title": title, "slug": slug, "content": content,
             "description": f"Best city tours and sightseeing in {city}: prices, top picks, and practical tips.",
