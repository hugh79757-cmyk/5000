...
def generate_nightlife_guide(topic):
    city = topic["city"]
  ...
  ## Tips for Nightlife in {city}"""
    prompt = f"""Write a nightlife and entertainment guide for {city}, {country}.
...
- Title must include "{city}" and relate to nightlife and entertainment
...
- Write as a nightlife guide and entertainment reviewer who knows every venue
...
Return ONLY the article in markdown starting with # title"""
    resp = _get_client().chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a nightlife guide and entertainment reviewer who knows every venue. STRICT RULES: 1) Never use: vibrant, bustling, hidden gem, treasure trove, must-visit, immerse yourself, embark, crystal-clear, world-class, bucket list, plethora, tapestry, myriad. 2) Format prices as whole numbers. 3) Never invent data. 4) Every section must include one practical tip. 5) Open with a specific concrete scene or fact."},
            {"role": "user", "content": prompt}
        ]
    )
    content = resp.choices[0].message.content.strip()
    title_match = re.match(r"^#\s+(.+)", content)
    title = title_match.group(1).strip() if title_match else topic.get("title", f"Nightlife And Entertainment in {city}")
    content = re.sub(r"^#\s+.+\n*", "", content, count=1).strip()
    slug = topic.get("slug", re.sub(r"[^a-z0-9]+", "-", city.lower()).strip("-") + "-nightlife")
    tags = [city, country, "Nightlife"] if country else [city, "Nightlife"]
    return {"title": title, "slug": slug, "content": content,
             "description": f"Best nightlife and entertainment in {city}: prices, top picks, and practical tips.",
