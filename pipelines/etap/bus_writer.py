...
Return ONLY the article in markdown starting with # title"""
    ...
    resp = _get_client().chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a budget travel blogger who rides buses across Europe. Write in first-person-informed tone. STRICT RULES: 1) Never use: plethora, vibrant, bustling, let\'s dive in, without further ado, hidden gem, tapestry, myriad, embark. 2) Format prices as whole numbers when .0. 3) Never invent data. 4) Every section must include one practical tip (station location, luggage policy, wifi availability, seat selection strategy). 5) Open with a concrete detail about this specific route."},
