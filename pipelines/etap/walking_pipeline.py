...
from pipelines.etap.walking_writer import generate_walking_guide
...
def _run_impl() -> bool:
    ...
    cover = fetch_city_image(city + " walking tour", country, article["slug"]) if city else None
    body = fetch_body_images(city + " walking tour", country, article["slug"], count=8) if city else []
...
def run_batch(count=1):
    ...
    can_pub, today_count = check_daily_quota("walking-hugo", max_per_day=5)
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[walking-hugo] daily quota reached ({today_count}/5)")
        return 0
