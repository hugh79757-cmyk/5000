"""cruise_pipeline.py - Shore Excursions blog pipeline"""
...
from pipelines.etap.cruise_writer import generate_cruise_guide

BLOG_ID = "cruise-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/cruise-hugo"
TOPIC_TABLE = "cruise_topics"
...
def run():
    ...
    body = fetch_body_images(city + " shore excursion", country, article["slug"], count=3) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"],
                        "shore excursions from " + city, 60, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"],
                        "shore excursions in " + country, 40, 1)
    return True
...
def run_batch(count=1):
    ...
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[cruise-hugo] daily quota reached ({today_count}/5)")
        return 0
