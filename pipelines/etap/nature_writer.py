"""nature_writer.py - Nature & Wildlife guide generator (v2: curated, deduplicated)"""
import logging
import os
import re
import sqlite3
...
def fetch_tours(city, country=None):
    ...
    rows = conn.execute("""
        ...
        FROM viator_tours
        WHERE city = ? AND category IN ('Nature and Wildlife Tours', 'Hiking Tours', 'Mountain Bike Tours', 'Safari')
...
def generate_nature_guide(topic):
    city = topic["city"]
    ...
    logger.info(f"[nature] {city}: {original_count} tours -> {dedup_count} after dedup")
    ...
    prompt = f"""Write a nature and wildlife tours guide for {city}, {country}.
...
## [Hook: one specific, concrete sentence about {city} nature]
## Top Nature and Wildlife Tours in {city}
...
8. NEVER use: plethora, vibrant, bustling, embark, tapestry, myriad, hidden gem, unforgettable, crystal-clear, soak in, immerse yourself, lets dive in, without further ado, a testament to, culinary delights, gastronomic, rich cultural heritage, seamlessly, breathtaking, brimming with, a must-visit, treasure trove, staggering"""
    ...
    title = title_match.group(1).strip() if title_match else topic.get("title", f"Best Nature Tours from {city}")
