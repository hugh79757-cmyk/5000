...
def run():
    topic = pick_topic()
    if not topic:
        logger.info("[michelin-hugo] No topics")
        return False
    logger.info(f"[michelin-hugo] {topic.get('city','')} generating")
    article = generate_michelin_guide(topic)
    if not article:
        return False

    # Quality guard
    _data_prices = []
    for _t in article.get("restaurants", []):
        try:
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"], "Michelin restaurants in " + city, 65, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"], "Michelin dining in " + country, 40, 1)
    return True

def run_batch(count=3):
    from pipelines.etap.topic_manager import check_daily_quota
    can_pub, today_count = check_daily_quota("michelin-hugo", max_per_day=5)
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[michelin-hugo] daily quota reached ({today_count}/5)")
        return 0
