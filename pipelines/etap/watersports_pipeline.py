"""watersports_pipeline.py - Water Sports blog pipeline"""
import logging
import os
import re
import sqlite3
...
from pipelines.etap.watersports_writer import generate_watersports_guide
...
BLOG_ID = "watersports-hugo"
SITE_PATH = "/Users/twinssn/Projects/ETAP/watersports-hugo"
TOPIC_TABLE = "watersports_topics"
...
def _run_impl() -> bool:
    ...
    article = generate_watersports_guide(topic)
    if not article:
    ...
    cover = fetch_city_image(city + " water sport", country, article["slug"]) if city else None
    body = fetch_body_images(city + " water sport", country, article["slug"], count=8) if city else []
    ...
    if city:
        register_entity("city", city, BLOG_ID, article["slug"],
                        "water sports in " + city, 60, 1)
    if country:
        register_entity("country", country, BLOG_ID, article["slug"],
                        "watersports in " + country, 40, 1)
    return True
...
def run_batch(count=1):
    ...
    can_pub, today_count = check_daily_quota("watersports-hugo", max_per_day=5)
    if not can_pub:
        import logging
        logging.getLogger(__name__).info(f"[watersports-hugo] daily quota reached ({today_count}/5)")
        return 0
