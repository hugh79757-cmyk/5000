...
def estimate_resale(base_price, brand, fuel_type, segment="", model=""):
    ...
    if base_price > 6000:
        ...
        price_factor = +2
    base_rate += price_factor
    jitter = random.uniform(-1.5, 1.5)
    base_rate = max(30, min(70, base_rate + jitter))
    yr1_drop = random.uniform(15, 22)
    yr2_drop = random.uniform(8, 13)
    yr3_drop = random.uniform(7, 12)
    r1 = round(100 - yr1_drop, 1)
    r2 = round(r1 - yr2_drop, 1)
    r3 = round(r2 - yr3_drop, 1)
    r3 = max(r3, base_rate - 3)
    r3 = min(r3, base_rate + 3)
    r2 = max(r2, r3 + 5)
    r1 = max(r1, r2 + 5)
    r1 = min(r1, 88)
    r2 = min(r2, r1 - 3)
    r3 = min(r3, r2 - 3)
    return {
        "resale_1yr": round(base_price * r1 / 100),
        "resale_2yr": round(base_price * r2 / 100),
        "resale_3yr": round(base_price * r3 / 100),
        "resale_rate_percent": round(r3),
    }
