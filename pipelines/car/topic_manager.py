...
def josa_wa(text) -> str:
    return "과" if has_batchim(text) else "와"

def josa_eul(text) -> str:
...
def select_topic(conn, site_id="hotissue", days_window=7, skip_ids=None, post_type=None):
    ...
    if post_type:
        topics = c.execute("""
            SELECT * FROM topics WHERE (status = 'pending' OR status = 'published') {_reuse} AND site_id = ? AND post_type = ? ORDER BY priority DESC, RANDOM()
        """.replace("{_reuse}", _reuse), (cutoff, site_id, post_type)).fetchall()
    else:
        topics = c.execute("""
            SELECT * FROM topics WHERE (status = 'pending' OR status = 'published') {_reuse} AND site_id = ? ORDER BY priority DESC, RANDOM()
        """.replace("{_reuse}", _reuse), (cutoff, site_id)).fetchall()
    ...
    return c.execute("""
        ...
        WHERE (t.status = 'pending' OR t.status = 'published') {_reuse} AND t.site_id = ?
        AND t.id NOT IN ({}){}
        ORDER BY p.published_at ASC NULLS FIRST
        LIMIT 1
    """.format(placeholder, " AND t.post_type = ?" if post_type else "").replace("{_reuse}", _reuse),
...
def validate_body(body, data):
    ...
    if fuel_eff and fuel_eff > 0 and str(fuel_eff) not in body:
