...
PALETTE = {
    "생활정보": {"bg": (41, 128, 185), "accent": (52, 152, 219)},
    "세금": {"bg": (39, 174, 96), "accent": (46, 204, 113)},
    "축제": {"bg": (192, 57, 43), "accent": (231, 76, 60)},
    "건강": {"bg": (142, 68, 173), "accent": (155, 89, 182)},
    "자동차": {"bg": (44, 62, 80), "accent": (52, 73, 94)},
    "default": {"bg": (52, 73, 94), "accent": (69, 90, 110)},
}
...
def generate_gap_thumbnail(title, category="생활정보", output_path=None):
...
def upload_thumbnail(title, category="생활정보"):
    try:
        from shared.r2_uploader import upload_file

        tmp_path = generate_gap_thumbnail(title, category)
        title_hash = hashlib.md5(title.encode()).hexdigest()[:10]
        r2_key = f"gap-thumbnails/{datetime.now().strftime('%Y%m%d')}-{title_hash}.webp"
        url = upload_file(tmp_path, r2_key, content_type="image/webp")
        os.remove(tmp_path)
        return url
    except Exception as e:
        logger.warning(f"GAP 썸네일 업로드 실패: {e}")
        return None
