...
CATEGORY_FILTERS = {
    ...
    "golf-hugo": {
        "allowed": ["골프", "골프클럽", "골프드라이버", "아이언", "아이언세트", "골프백", "골프거리측정기", "골프화", "퍼터", "골프공", "골프장갑", "골프의류", "골프우산", "스윙연습기", "골프장", "골프공", "골프용품", "골프연습", "골프티", "골프GPS", "골프모자"],
        "blocked": ["도서", "교재", "인형", "장난감", "식품", "완구", "공구", "전동드라이버", "드릴"],
    },
    "bike-hugo": {
...
"blocked": ["도서", "교재", "필기", "실기", "기능사", "자격증",
    ...
    "health-hugo": {
        "allowed": ["건강", "영양", "비타민", "멀티", "종합", "유산균", "루테인", "오메가", "콜라겐",
    ...
    "kitchen-hugo": {
        "allowed": ["냄비", "프라이팬", "주방", "식기", "칼", "도마", "용기", "정리",
                     "텀블러", "도시락", "밀폐", "보관", "조리도구",
                     "가위", "저울", "타이머", "주걱", "냄비받침",
    ...
    # 재테크 실물 도구 계열. allowed 토큰 = KEYWORD_MAP 키워드가 반드시 포함해야 하는 부분 문자열.
    "finance-hugo": {
        "allowed": ["전자계산기", "가계부"],
        "blocked": ["도서", "교재", "인형", "장난감", "식품", "완구"],
    },
    "stock-hugo": {
        "allowed": ["금고", "돈통", "현금정리", "전자계산기"],
        "blocked": ["도서", "교재", "인형", "장난감", "식품", "완구"],
    },
    "dividend-hugo": {
        "allowed": ["가계부", "서류정리"],
    ...
    "etf-hugo": {
        "allowed": ["전자계산기", "명함", "가계부"],
        "blocked": ["도서", "교재", "인형", "장난감", "식품", "완구"],
    },
    "sector-hugo": {
        "allowed": ["금고", "전자계산기", "현금정리", "돈통"],
        "blocked": ["도서", "교재", "인형", "장난감", "식품", "완구"],
    },
    "ipo-hugo": {
        "allowed": ["저금통", "전자계산기", "가계부", "돈통"],
        "blocked": ["도서", "교재", "인형", "장난감", "식품", "완구"],
    },
    # 시니어 복지 계열 (식품/건강식품은 허용 — coupang_senior 검증 카테고리)
    "senior-hugo": {
        "allowed": ["혈압계", "지팡이", "보행보조기", "보행기", "실버카", "욕조손잡이",
                     "욕실손잡이", "안전손잡이", "미끄럼방지", "성인용기저귀", "기저귀",
                     "찜질기", "온열", "시니어", "노인", "어르신", "실내자전거", "스텝퍼",
                     "확대경", "이동변기"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구"],
    },
    "senior-blogger": {
        "allowed": ["혈압계", "지팡이", "실버카", "안전손잡이", "미끄럼방지",
                     "성인용기저귀", "기저귀", "찜질기", "온열", "시니어", "노인",
                     "어르신", "확대경", "이동변기", "스텝퍼", "실내 자전거",
                     "오메가3", "영양제", "홍삼"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구"],
    },
    # ===== CAP (car pipeline) Phase 2B (2026-08-23) — 자동차 계열 (additive) =====
    "compare-hugo": {
        "allowed": ["차량", "자동차", "블랙박스", "하이패스", "타이어", "세차",
                     "차박", "차량용", "차량용품", "엔진오일", "와이퍼", "루프박스",
                     "코팅제", "시트", "핸들", "대시보드", "트렁크", "네비게이션",
                     "카시트", "거치대", "방향제", "점프스타터", "배터리", "매트"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "deal-hugo": {
        "allowed": ["차량", "자동차", "차량용", "차량용품", "블랙박스", "타이어",
                     "하이패스", "거치대", "충전", "시거잭", "인버터", "세차",
                     "와이퍼", "휠", "번호판", "실리콘", "방향제", "쿨러", "정리함",
                     "카시트", "대시보드"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "ev-hugo": {
        "allowed": ["전기차", "EV", "충전", "충전기", "배터리", "카시트", "차량",
                     "자동차", "충전 케이블", "차박", "매트", "블랙박스", "어댑터",
                     "타이어", "와이퍼", "에어컨 필터", "썬팅", "발판", "트렁크",
                     "인버터", "방음"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "guide-hugo": {
        "allowed": ["차량", "자동차", "첫차", "초보", "카시트", "주차", "미러",
                     "베개", "쿠션", "정리함", "컵홀더", "매트", "안전", "시트",
                     "LED", "조명", "클리너", "방향제", "거치대", "필수용품",
                     "차량용", "차량용품"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "hotissue-hugo": {
        "allowed": ["차량", "자동차", "신차", "스마트 키", "필터", "워셔액", "첨가제",
                     "브레이크", "냉각수", "부동액", "코팅", "왁스", "휠", "탈취",
                     "살균", "스노우 체인", "타이어", "OBD", "진단기", "배터리",
                     "블랙박스", "차량용", "차량용품"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "tco-hugo": {
        "allowed": ["차량", "자동차", "엔진 오일", "배터리", "타이어", "연비",
                     "하이브리드", "점화 플러그", "필터", "브레이크", "정비", "툴",
                     "토크 렌치", "연료 첨가제", "냉매", "청소", "공구", "오일",
                     "부품", "차량용품", "차량용"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "rank-hugo": {
        "allowed": ["차량", "자동차", "인기", "베스트", "블랙박스", "타이어", "세차",
                     "액세서리", "매트", "루프박스", "차박", "스노우체인", "시트",
                     "핸들", "와이퍼", "코팅제", "방향제", "냉장고", "네비게이션",
                     "충전기", "카시트", "차량용", "차량용품"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "pick-hugo": {
        "allowed": ["차량", "자동차", "카시트", "거울", "선풍기", "충전", "펌프",
                     "청소", "클리너", "시트", "마사지", "베개", "담요", "거치대",
                     "가방", "노트북", "라이브캠", "차량용", "차량용품", "에어 펌프",
                     "스팀 청소기"],
        "blocked": ["도서", "교재", "인형", "장난감", "완구", "의류", "화장품", "식품"],
    },
    "best-kitchen-hugo": {
        "allowed": ["키친", "키친타올", "타올", "랩", "위생", "종이컵", "장갑", "위생장갑", "위생롤백", "냄비", "프라이팬", "주방", "식기", "칼", "도마", "용기", "정리",
                     "텀블러", "도시락", "밀폐", "보관", "조리도구",
                     "가위", "저울", "타이머", "주걱", "냄비받침",
                     "에어프라이어", "전기냄비", "밥솥", "믹서기", "전기포트",
                     "커피머신", "에스프레소", "캡슐커피", "원두", "원두커피", "드립",
