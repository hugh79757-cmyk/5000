...
[interior-hugo 전용 규칙]
- 가구/인테리어 소품 위주로 작성.
- 비교표에 반드시 크기(cm), 소재, 색상 옵션 수를 포함할 것. 수치 없으면 해당 셀 "-"로 표기.
- 각 상품 소개에 가로×세로×높이(cm) 치수를 명시할 것. 스펙 데이터에 없으면 생략.
- 로켓배송 상품이 없는 경우 CTA에서 로켓배송 언급 금지. 대신 "설치 서비스 포함 여부를 확인하세요" 또는 "배송 전 사이즈 재측정을 권장합니다"로 대체.
- 구체적인 생활 장면으로 추천 서술. "신혼부부" 대신 "이사 후 거실 가구를 처음 맞추는 신혼부부", "자취생" 대신 "원룸 12평에 소파와 TV장을 함께 배치해야 하는 자취생" 처럼 공간 크기와 상황을 구체화.
""",
    "fitness-hugo": """
[fitness-hugo 전용 규칙]
...
6. **정리 (H2)**:
   ...
   - 행동 유도(CTA): 각 페르소나별 추천 제품명만 제시. "쿠팡" 단어 반복 금지. 소개한 상품 중 로켓배송 상품이 1개 이상 있을 때만 "로켓배송 표기 제품을 우선 고려하세요" 문구 사용. 로켓배송 상품이 없으면 해당 문구 사용 금지.
   - "마무리", "결론" 소제목 금지.
