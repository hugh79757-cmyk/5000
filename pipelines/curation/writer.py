...
[interior-hugo 전용 규칙]
- 가구/인테리어 소품 위주로 작성.
- 비교표는 최대 5열까지만 허용 (기본: # | 상품명 | 가격 | 핵심 특징). 크기/소재 등 추가 정보는 데이터가 3개 이상 있을 때만 1열로 추가하고, 정보 없으면 열 자체를 생략 — "-" 사용 금지.
...
- 비교표는 최대 5열까지만 허용 (기본: # | 상품명 | 가격 | 핵심 특징). 성분/함량 등 추가 정보는 데이터가 3개 이상 있을 때만 1열로 추가하고, 정보 없으면 열 자체를 생략 — "-" 사용 금지.
...
- 비교표는 최대 5열까지만 허용 (기본: # | 상품명 | 가격 | 핵심 특징). 적합 동물/체중 등 추가 정보는 데이터가 3개 이상 있을 때만 1열로 추가하고, 정보 없으면 열 자체를 생략 — "-" 사용 금지.
- 로켓배송 상품이 없는 경우 CTA에서 로켓배송 언급 금지.
""",
    "kitchen-hugo": """
[kitchen-hugo 전용 규칙]
- 각 상품 소개에 용량(L) 또는 크기(cm), 소재, 호환 열원(가스/인덕션/전기)을 명시할 것.
- 비교표는 최대 5열까지만 허용 (기본: # | 상품명 | 가격 | 핵심 특징). 용량/소재 등 추가 정보는 데이터가 3개 이상 있을 때만 1열로 추가하고, 정보 없으면 열 자체를 생략 — "-" 사용 금지.
...
- 비교표는 최대 5열까지만 허용 (기본: # | 상품명 | 가격 | 핵심 특징). 피부 타입/성분 등 추가 정보는 데이터가 3개 이상 있을 때만 1열로 추가하고, 정보 없으면 열 자체를 생략 — "-" 사용 금지.
...
- 비교표는 최대 5열까지만 허용 (기본: # | 상품명 | 가격 | 핵심 특징). 무게/크기 등 추가 정보는 데이터가 3개 이상 있을 때만 1열로 추가하고, 정보 없으면 열 자체를 생략 — "-" 사용 금지.
...
2. "한눈에 보는 비교표" — `| # | 상품명 | 가격 | 핵심 특징 |` 4열 기본, 최대 5열까지만 허용 (6열 이상 금지, "-" 사용 금지, 정보 없으면 열 생략)
