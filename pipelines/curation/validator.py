"""curation(상품 큐레이션) 파이프라인 표준 골격 — validator (Phase 61, D-01).

표준 6-모듈 골격(pipeline/fetcher/topic_manager/writer/enrich/validator)의 일부.
curation 분기는 독립적인 validator 모듈이 없었고, 검증이 `shared.validators` 의
`validate_post()` 호출로 pipeline.py 에 인라인되어 있다.
...
def validate_post(cfg, article):
    """발행 전 검증 — `shared.validators.validate_post` 를 위임.
    ...
    """
    from shared.validators import validate_post
    ...
    issues = validate_post(blog_id, title, body_md, ctx)
