...
- H2 3(신청 방법): 신청 절차를 H3(###)로 단계별 세분화하거나, 순서가 있는 목록으로 정리하세요. "### 신청 준비물" 같은 H3 아래에서는 "- 구비서류 OOO 준비" 형태의 대시(-) 목록을 사용하고, 구비서류·신청 채널·방문처를 빠짐없이 나열하세요. "[ ]", "[x]" 같은 태스크리스트 문법은 절대 사용하지 마세요 — 일반 대시 목록으로만 작성하세요.
