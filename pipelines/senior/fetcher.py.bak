...
CATEGORIES = {
    "의료지원": ["임플란트", "틀니", "건강검진", "치매", "요양", "의료", "진료", "백신", "건강보험"],
    "돌봄서비스": ["돌봄", "장기요양", "방문요양", "주간보호", "재가"],
    "교통복지": ["교통", "지하철", "버스", "KTX", "경로우대"],
    "일자리금융": ["일자리", "고용", "취업", "대출", "금융", "보험", "연금"],
    "문화여가": ["문화", "여가", "스포츠", "바우처", "누리카드", "관광", "박물관"],
    "연금생활지원": ["기초연금", "에너지", "난방", "주거급여", "생계급여", "보조금"],
}
...
def fetch_senior_services(page=1, per_page=100):
    """공공서비스(혜택) API에서 시니어 관련 서비스 조회"""
    logger.info("공공서비스 혜택 API 조회 시작")
    all_services = []

    try:
        for p in range(1, 4):  # 최대 3페이지 (300건)
            params = {
                "page": p,
                "perPage": per_page,
                "serviceKey": API_KEY,
...
def fetch_all(max_pages=10):
    """모든 시니어 데이터 수집 — 여러 페이지 순회"""
    services = []
    for page in range(1, max_pages + 1):
        page_services = fetch_senior_services(page=page)
