...
PATTERNS = {
    ...
    "ko_thinking": {
        "regex": r"사용자가 제공한 데이터|절대\s+.*\s+말라고 했습니다|초안:|이제\s+.*\s+작성|H2-\d|문장 수:|주의:|규칙|~해야 합니다\.",
    ...
    "prompt_instruction_leak": {
        "regex": r"bold-list로 정리|테이블 금지|1~2문장으로 소개|H2 없이|최소 \d+문장|~를 명시해야|금지 표현|체크포인트|구조:|도입부 \(|도입부 \(H2 없이|본문에 반드시|비교 분석에 활용|비교하세요|비교 수치를 언급|단독 분석으로 작성|비교 표에|절대로 다른 차량|임의로|비교 대상으로",
        "lang": "ko", "severity": "high"
    },
    # 5. 금지어 역노출 — 프롬프트가 금지한 표현이 본문에 등장
    "forbidden_word_reverse": {
        "regex": r"확인해 보시기 필요합니다|보시기 필요합니다|바랍니다|되시길|있으시|마무리하며|마치며|정리하며|알아보겠습니다|과연|놀랍게도|충격적으로",
