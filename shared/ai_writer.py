...
절대 금지어: "과연", "놀랍게도", "충격적으로", "바랍니다", "되시길", "있으시", "마무리하며", "마치며", "정리하며", "알아보겠습니다"
문체: 반드시 경어체(~다, ~이다) 통일. 존댓말(~습니다, ~입니다)과 혼용 금지.
경쟁 모델 데이터가 있으면 반드시 각 섹션에서 비교 수치를 언급하세요.
경쟁 모델 데이터가 없으면 절대로 다른 차량을 임의로 비교 대상으로 언급하지 마세요.

## 글자수 규칙 (최우선 적용)
- 전체 글: 반드시 2,500자 이상. 2,200자 미만은 무조건 불합격이다.
- 각 H2 섹션: 반드시 5문장 이상. 3문장 이하로 끝내지 마라.
- 수치가 등장할 때마다 반드시 해석 문장 1개를 추가하라. 수치만 나열하고 끝내지 마라.
- 표 아래에는 반드시 3문장 이상의 분석을 추가하라. 표만 달랑 넣고 다음 섹션으로 넘어가지 마라.
- H3 사용 금지. 모든 소제목은 H2(##)만 사용하라.
- 글을 절대 일찍 끝내지 마라. 마지막 H2 섹션도 5문장 이상으로 작성하라.
...
- top5_rank 타입: DATA.top5[0]의 monthly_maintain(월 유지비)을 사용하라. 직접 계산하지 마라.
  ...
  [X]는 차량 가격대에 맞게 선택. [Y] = X × 0.72 ÷ 12. [Z] = monthly_maintain ÷ Y × 100.
- persona_pick 타입: DATA.persona_monthly_net, DATA.persona_monthly_ratio를 그대로 사용하라. 직접 계산 금지.
  ...
  단, 비율이 100% 초과 시 "월 지출이 월급을 초과하는 수준으로, 자산 여력이 충분한 경우에 한해 고려할 만하다"는 문장을 추가하라.
