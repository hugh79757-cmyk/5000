...
def _check_car(title: str, body: str, ctx: dict) -> list:
    ...
    # 제목에 차량명 포함 여부 확인
    import re
    # 차량명 리스트: car.db에서 자동 로드 + 브랜드명
    import sqlite3 as _sq
    _car_db = _sq.connect("/Users/twinssn/Projects/5000/data/car.db")
    _car_db.row_factory = _sq.Row
    _db_models = [r[0] for r in _car_db.execute("SELECT DISTINCT model FROM cars").fetchall()]
    _db_brands = [r[0] for r in _car_db.execute("SELECT DISTINCT brand FROM cars").fetchall()]
    _car_db.close()
    # 브랜드 별칭 (DB에 없지만 제목에 등장 가능)
    _brand_aliases = ["쉐보레", "쌍용", "KG", "폭스바겐", "포르쉐", "BYD", "폴스타", "랜드로버"]
    _car_brands = list(set(_db_models + _db_brands + _brand_aliases))
