...
def _get_today_count(blog_id: str) -> int:
    ...
    try:
        ...
        logger.exception(f"[DB_ERROR] Failed to get today count for blog {blog_id}: {e}")
        return 0
...
def is_korean_content(text: str, min_hangul_ratio: float = 0.5) -> bool:
    """한국어 콘텐츠 검증 — 한글 vs 중국어 비율 확인"""
    if not text:
        return True
    import re
    hangul = len(re.findall(r"[\uAC00-\uD7AF]", text))
    len(re.findall(r"[\u4E00-\u9FFF]", text))
    total  = len(re.findall(r"[\uAC00-\uD7AF\u4E00-\u9FFF]", text))
    if total == 0:
        return True
    ratio = hangul / total
    return ratio >= min_hangul_ratio
...
def _check_stap(title: str, body: str, ctx: dict) -> list:
    ...
    if is_dividend:
        zero_patterns = [(p, l) for p, l in zero_patterns if l != "0%"]
    ...
    for _idx, _line in enumerate(_lines):
        if _line.startswith("## "):
            ...
            for _k in range(_idx + 1, min(_idx + 5, len(_lines))):
                _s = _lines[_k].strip()
                if _s and not _s.startswith("## ") and not _s.startswith("#") and _s not in {"---", ">"}:
                    _has_content = True
                    break
            if not _has_content:
