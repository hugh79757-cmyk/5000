...
# 섹션 4: Hugo 템플릿 수정 규칙 (재발 방지)

> 최종 갱신: 2026-07-25
> 적용 대상: 모든 5000 브랜치 블로그 (CAP/TAP/STAP/CUAP/SEAP/RAP Hugo 사이트)
> 작성자: 템플릿 수정 시 CSS 골격/기능 소실 회귀를 방지하기 위함

## 절대 규칙 (MUST)

Hugo 레이아웃 템플릿(`single.html`, `list.html`, `baseof.html` 등)을 수정할 때는 반드시 아래 절차를 따른다. 위반 시 템플릿 구조 손실로 인한 기능 회귀(타이틀 미노출, 차트/관련글/페이지네이션 소실)가 발생한다.

### 1. 원본 기준선 확보 (Baseline)

변경 전에 **테마의 원본 템플릿 파일**을 반드시 읽고 기준으로 삼는다.

```bash
# 원본 확인 (예: Blowfish single.html)
cat /Users/twinssn/Projects/shared-themes/blowfish/layouts/_default/single.html
```

- `themes/` 디렉토리의 심볼릭 링크를 따라 원본 파일을 직접 읽을 것
- **절대 처음부터 새로 작성하지 말 것** — 반드시 원본을 복사한 후 증분(increment) 수정

### 2. 증분(Increment) 방식으로만 수정

원본 템플릿을 그대로 복사한 후, 변경이 필요한 부분만 추가/수정한다:

- **Bad** — 처음부터 새로 작성 (❌ 지금까지의 실수 패턴)
  ```html
  {{ define "main" }}
  <article>
    <header>
  ...
  {{ end }}
  ```
  → Hero, 메타데이터, TOC, 관련글, 페이지네이션, 시리즈 등 **통째로 소실**

- **Good** — 원본 복사 후 증분 추가 (✅ 올바른 방식)
  ```html
  {{ define "main" }}
    ...
    <article>
      ...
      <header id="single_header">
        {{ partial "adsense/top.html" . }}   ← ★ 여기에만 추가
        {{ if .Params.showBreadcrumbs ... }}
          {{ partial "breadcrumbs.html" . }}
        {{ end }}
        <h1 class="mt-0 text-4xl ...">{{ .Title | emojify }}</h1>
        ...
      </header>
      {{ partial "adsense/in-article.html" . }}  ← ★ 여기에만 추가
      {{/* Body */}}                             ← 원본 유지
      ...
    </article>
  {{ end }}
...
수정 후 반드시 원본과의 차이점(diff)을 확인하여 의도하지 않은 삭제가 없는지 검증한다:

```bash
# 1. 원본을 temp 파일로 백업
cp themes/blowfish/layouts/_default/single.html /tmp/single_original.html

# 2. 수정 후 diff 비교
diff /tmp/single_original.html layouts/_default/single.html
```

diff 출력에서 **추가된 라인(ad 슬롯 등) 외에 삭제된 라인이 있으면 즉시 수정**한다. 예를 들어 `hero/`, `article-meta`, `related.html`, `article-pagination`, `series/`, `sharing-links`, `prose` 클래스 등이 삭제되지 않았는지 반드시 확인한다.

### 4. 변경 범위 제한 (One-Edit Rule)

- **단일 템플릿 파일당 변경 포인트는 최대 3곳 이하**로 제한한다
- 3곳 이상 수정이 필요한 경우, 작업을 여러 Phase로 분할한다
- 이유: 다중 변경은 diff 검증을 어렵게 만들고 의도치 않은 소실 위험을 높임

### 5. 수정 가능한 항목 (화이트리스트)

다음 항목만 템플릿에 추가/수정할 수 있다. 이 외의 요소는 원본을 절대 건드리지 않는다:

| 허용 항목 | 예시 |
|----------|------|
| AdSense 슬롯 partial 추가 | `{{ partial "adsense/top.html" . }}` |
| `showTableOfContents` 비활성화 | `showTableOfContents = false` |
| 커스텀 CSS 클래스 추가 (ad 전용) | `class="ad-top"`, `.ad-top { ... }` |
| 광고 컨테이너 div | `<div class="ad-top not-prose my-4">` |

---

<!-- GSD:subagent-stuck-detection-start -->
