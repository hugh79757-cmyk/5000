...
## 시간대 라벨 정정 — 안 1 채택 (2026-09-15 12:40 로컬(+07), 사용자 m0290)

- **예외 발견**: 호스트 시간대 = Asia/Bangkok **+07** (KST +09 아님). /etc/localtime
  실측 + REG#4 run created 01:24:59Z vs 로컬 08:24 독립 검증 → 오프셋 +7 확정.
- **초판 cron 매핑 폐기**: `30 21/10 1/35 4/30 8/45 11` (+09 전제) →
  **신규 `30 23/10 3/35 6/30 10/45 13`** (+07 기준, 사용자 검산 완료).
  슬롯 시각(cap.yaml)은 현행 유지 — cap.yaml 무수정.
- **표기 규약**: 활성 문서 "KST" → "로컬(+07, Asia/Bangkok)". 과거 worklog
  재작성 금지 — G-C-REQUEST 상단 규약 노트로 대체 ("2026-09-15 이전 KST 라벨은
  로컬(+07) 오기록").
- **quota 리셋 재기록**: UTC 자정 = 로컬(+07) 07:00 (초판 "KST 07:00" 라벨만 오기,
  관측 자체는 정확).
- **안 2(진짜 KST 발행) 이월**: 이관 완료 후 트래픽 최적화 단계 단일변수 실험
  (사용자 재승인 대상).
- 사전조건 1(원장 원값: Mac publish_ledger +07 / R2본 stap_content UTC /
  쿼터 가드 content_store.py:118 UTC 프레임 일관)·2(Mac R2 put 0건 무충돌)·
  3(schedule 시 dry 경로 부재) 완료.
- G-C-REQUEST.md 정정 완료 + publish.yml concurrency `publish-lock-car` 전환 +
  cron 5줄 신규 매핑 주석 (활성화 = 금일 18:00~20:15 로컬 커밋 창).
