...
## 핸드오프 갱신 — 사용자 판정 확정 (2026-09-14)

**판정 채택: 격리 유지·통합 폐기 확정 (D-04 유지).**

- "모든 블로그를 한곳에서 관리"는 이미 달성 — 5000 dispatcher가 STAP/TAP 포함 전 함대의 발행·스케줄·원장을 통제(경계 JSON 1개 왕복, T2). 관리의 단일 지점은 물리적 코드 위치가 아니라 통제 구조.
- 물리 병합은 openai 메이저 분기(17티어 폴백 체인 전 재검증)와 원장 병합 리스크만 추가. D-04를 뒤집는 게 아니라 유지하는 결정 → ADR 절차 불필요.
- 이관 관점: 격리 구조는 걸림돌 아님 — 러너에서 venv 설치 스텝만 추가하면 서브프로세스 호출은 Mac과 동일하게 재현. "구조 그대로 이관"으로 전체 일정 2-4주 단축.

### 확정 사항 4건

1. **Phase 80 종결** — 판정 "격리 유지·통합 비추천" 채택. 핸드오프 "통합 판정 대기 중" → "통합 폐기 확정(D-04 유지)".
2. **이관 순서 기존 합의 복귀** — G-B 관찰(현재 2/5) → Task 9(BUG-12·14: publisher.py:832 스코프화 + precheck 14→30일 — 통합 없는 정확한 해결점) → G1~G3 → G4(ETAP)+G-STAP/TAP 병렬(별도 게이트).
3. **G-STAP/TAP 이관 설계 반영** — ① 러너 venv 설치+호환 스텝 ② STAP계열 원장(STAP/data/stap_content.db 19MB)·tap.db(19MB)를 해당 그룹 이관 시 round-trip 대상에 추가 — 5000/data/stap_content.db(124MB, 5000계열)와 동명이므로 동명 함정 문서화 선행(부산물 2) ③ STAP_ROOT/TAP_ROOT 러너 경로 오버라이드 확인.
4. **감시 항목 등록** — openai 버전 분기(3.3.1/2.37/2.44): 격리 유지 시 무해(각자 자기 venv)하나, 추후 어느 쪽이든 openai 대규모 업그레이드 시 폴백 체인 회귀 테스트 필요.
